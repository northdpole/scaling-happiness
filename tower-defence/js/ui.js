		...
		click 				: function(x, y) {
			...
			if(this.map.layout[yCell][xCell] != 1){			
				...
				if(clickedTurret){
					...
					this.turretButtons[0].text = (clickedTurret.lvl + 1 )  + " Level.";
					...
					if(clickedTurret.lvl < 5 && this.player.money > Math.floor(clickedTurret.cost/2)){
						...
						this.turretButtons[1].text = "Improve: " + Math.floor(clickedTurret.cost/2);
					} else if (clickedTurret.lvl < 5) {
						this.turretButtons[1].text = "Improve: " + Math.floor(clickedTurret.cost/2);
						this.turretButtons[1].captureClick = false;
					} else {
						this.turretButtons[1].text = "Improve."
						this.turretButtons[1].captureClick = false;
					}
					this.turretButtons[2].text = "Sell: " + Math.floor(clickedTurret.cost/3);
		...
		addButton(this.towerButtons,0,0,wt,ht,true,true,true,this.cost,this.type,0,0, this.text, 
			...
			function(i,x,y){
				...
				this.info = {
					...
					text 	: [
						"Dammage: " + this.towerTypes[this.towerButtons[i].type].damage + ", Radius: " + this.towerTypes[this.towerButtons[i].type].range,
						"Recharge: " + this.towerTypes[this.towerButtons[i].type].cooldown / 1000 + "с.",
						"Available с " + (this.towerTypes[this.towerButtons[i].type].fromLvl + 1) + " level."
					],
					multiLine : true,
				}
	...
	for (i in this.activeStack) {
		if (typeof this.activeStack[i] != 'undefined' && this.activeStack[i].visible) {
			...
			if(this.activeStack[i].hasOwnProperty("cost")){
				if(this.player.money >= this.activeStack[i].cost && this.activeStack[i].text != 'improve.'){
